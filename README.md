...
- `suppress-movement` (default: false): Prevent cursor movement while gesture recognition is active by replacing the configured X/Y event values with 0.
...
Neither the number of patterns, their length, nor the number of bindings per pattern is capped. Each processor sizes its gesture trie from its own devicetree children. Movement events are processed directly; only matched gesture execution is deferred.
