...
- `suppress-movement` (default: false): Suppress cursor movement events while gesture recognition is active. X/Y events configured by `event-code-x` and `event-code-y` are zeroed in place. (They used to be consumed by returning `ZMK_INPUT_PROC_STOP`, which aborted the rest of the processor chain and, on ZMK's input-listener layer-override path, was discarded entirely - so the pointer kept moving. Writing zero works from anywhere in the chain.)
...
- `active-layers` (default: 0, meaning every layer): Bitmask of the layers this instance acts on. Several instances can then share one input listener's base chain, with this mask - not a layer override - deciding which acts. That keeps the layer-to-set mapping changeable at runtime; see `zmk/mouse_gesture.h`.
- `display-name`: Label for configuration UIs; defaults to the node name.
